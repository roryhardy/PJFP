...
class PJFP {
	...
	private function curl() {
		$ch  = curl_init();
		$url = sprintf("http://picasaweb.google.com/data/feed/base/user/%s/albumid/%s?alt=json&fields=entry(media:group)&imgmax=%u%s",
			$this -> config['user'],
			$this -> album_id,
	...
	private function socket() {
		...
		if (!$fp)
			...
			$out = sprintf("GET /data/feed/base/user/%s/albumid/%s?alt=json&fields=entry(media:group)&imgmax=%u%s HTTP/1.1\r\n",
				$this -> config['user'],
				$this -> album_id,
				$this -> config['max_width'],
				(!empty($this -> auth_key) ? "&authkey={$this -> auth_key}" : "")
